...
| `--ephemeral`                | Run a one-shot task without creating or saving an agent          |
...
`--agent` defaults to `LETTA_AGENT_ID` when available. If you omit `--conversation`, each fire starts a new conversation.
...
- `--conversation new`: start a new conversation for every fire (default when omitted)
...
See [Memory](/concepts/memfs/index.md) for memory and MemFS details, and [Skills](/configuration/skills/index.md) for how skills use these commands. See [Teleportation](/platform/computers/teleportation/index.md) for moving a conversation between computers. The legacy `letta memfs` subcommand remains an alias for `letta memory`.
