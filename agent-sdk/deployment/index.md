...
`environment` accepts a name or selector object:
...
Use `deviceId` or an environment `id` to save a selection across reconnects. Use `connectionId` only for the current live connection.
...
`requestTimeoutMs` controls how long the SDK waits for an App Server protocol response or turn before timing out.
