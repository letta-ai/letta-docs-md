...
| Method                                      | Description                                                                                         |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `client.createAgent(options?)`              | Create a new stateful agent                                                                         |
...
| `cloud`  | None             | `apiKey`, `computer`, `sandbox`, `requestTimeoutMs`, `webSocketAuth`                   |

`requestTimeoutMs` controls how long the SDK waits for a protocol response or turn. The older `environment` option is a deprecated alias for `computer`. Browser and React Native applications should import the portable client from `@letta-ai/letta-agent-sdk/client`; see [Browser and React Native](/agent-sdk/browser/index.md).
...
| Method                           | Use                                                                        |
| -------------------------------- | -------------------------------------------------------------------------- |
| `send()`                         | Send a user message, optionally with a caller-supplied `otid`              |
...
interface SendOptions {
  otid?: string; // caller-supplied lineage id; generated when omitted
}
...
Options for `client.createAgent()` - these configure the stateful agent:
...
interface CreateSessionOptions {
  ...
  // Runtime controls
  cwd?: string;
  stateless?: boolean; // run without loading or changing the agent's MemFS
...
interface LettaCodeClientSessionOptions extends CreateSessionOptions {
  computer?: ComputerSelector; // Cloud only; `environment` is a deprecated alias
